...
app.post("/upload", upload.single("image"), async (req, res) => {
  ...
  if (!file || !reportId) {
    res.status(400).json({ success: false, message: "Missing file or reportId" });
    return;
  }
...
app.get("/api/get-map-levels", async (req, res) => {
  try {
    ...
    console.error("Error querying data:", err instanceof Error ? err.message : err);
...
app.get("/api/get-open-source-data", async (req, res) => {
  try {
    ...
    console.error("Error fetching open-source data:", err instanceof Error ? err.message : err);
...
app.post("/api/report-lead", async (req, res) => {
  ...
  try {
    ...
    console.error("Error inserting lead report:", err instanceof Error ? err.message : err);
